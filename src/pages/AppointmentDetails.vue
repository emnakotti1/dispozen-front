<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-if="isError" class="text-center py-12">
      ...
      <p class="text-gray-600 mb-4">Impossible de charger les détails de la réservation.</p>
      <button 
        @click="router.go(-1)"
    ...
    <!-- Appointment Details -->
    <div v-else-if="appointment" class="bg-white shadow-lg rounded-lg overflow-hidden">
      <!-- Header -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="space-y-6">
            <div>
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                <h3 class="font-medium text-gray-900 text-lg">{{ appointment.service.name }}</h3>
                <p v-if="appointment.service.description" class="text-gray-600 mt-2">{{ appointment.service.description }}</p>
                <div class="mt-3 flex items-center justify-between">
                  <span class="text-sm text-gray-500">Durée: {{ appointment.service.duration }} minutes</span>
                  <span class="text-lg font-semibold text-indigo-600">{{ appointment.service.price }}{{ appointment.service.currency }}</span>
                </div>
              </div>
            </div>
                ...
                <div class="flex items-center">
                  <div class="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center">
                    <span class="text-indigo-600 font-medium text-lg">
                      {{ getInitials(appointment.provider) }}
                    </span>
                  </div>
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600">{{ appointment.provider.email }}</p>
                    <p v-if="appointment.provider.phoneNumber" class="text-gray-600">{{ appointment.provider.phoneNumber }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="space-y-6">
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div v-if="appointment.calendar">
                    <span class="block text-sm font-medium text-gray-700 mb-1">Heure</span>
                    <p class="text-gray-900">
                      {{ appointment.calendar.startTime }} - {{ appointment.calendar.endTime }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div v-if="appointment.notes">
              <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1-8l-1-1a2 2 0 011.414-1.414L14 1l1 1-1 1-1.414-1.414A2 2 0 0111 2z"></path>
                </svg>
                Notes
              </h2>
              <div class="bg-gray-50 rounded-lg p-4">
            ...
            <!-- Contact Information -->
            <div v-if="appointment.provider.address">
              <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                ...
                <p v-if="appointment.provider.city" class="text-gray-900">
    ...
    <div v-else class="text-center py-12">
      ...
      <h3 class="text-lg font-medium text-gray-900 mb-2">Réservation introuvable</h3>
      <p class="text-gray-600 mb-4">Cette réservation n'existe pas ou a été supprimée.</p>
      <button 
...
const getStatusClass = (status: string) => {
  ...
  switch (status) {
    case 'confirmed': return `${baseClass} bg-green-100 text-green-800`
